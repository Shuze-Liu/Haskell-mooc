...
valAtRoot (Node x t1 t2) = Just x
-- valAtRoot (Node x Empty Empty) = Just x
...
treeSize :: Tree a -> Int
treeSize Empty = 0
treeSize (Node _ tl tr) = 1 + treeSize tl + treeSize tr
...
treeMax Empty = 0
treeMax (Node x tl tr) = maximum [x, treeMax tl, treeMax tr]
...
allValues condition (Node x tl tr) = condition x && allValues condition tl && allValues condition tr
...
cull val (Node x tl tr) = 
  if x == val then
    Empty
  else
    Node x (cull val tl) (cull val tr)
...
isOrdered (Node x (Node lx ll lr) (Node rx rl rr)) = x > lx && x < rx && isOrdered (Node lx ll lr) && isOrdered (Node rx rl rr)
isOrdered (Node x Empty (Node rx rl rr)) = x < rx && isOrdered (Node rx rl rr)
isOrdered (Node x (Node lx ll lr) Empty) = x > lx && isOrdered (Node lx ll lr)
...
walk [] (Node x lt rt) = Just x
walk (StepL:xs) (Node x lt rt) = walk xs lt
walk (StepR:xs) (Node x lt rt) = walk xs rt
...
set [] val (Node x lt rt) = Node val lt rt
set (StepL:xs) val (Node x lt rt) = Node x (set xs val lt) rt
set (StepR:xs) val (Node x lt rt) = Node x lt (set xs val rt)
