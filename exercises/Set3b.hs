...
mysum :: Int -> Int
mysum i = if 0 == i then 0 else i + mysum (i - 1)

sums' :: [Int] -> Int -> [Int]
sums' results 0 = results
sums' results i = sums' (mysum i : results) (i - 1)
...
indexDefault (x:xs) i def = if i > 0 then indexDefault xs (i - 1) def else def
...
-- sumOf' sum results [] = results
-- sumOf' sum results (x:xs) = sumOf' (sum + x) ((sum + x) : results) xs

sumOf' :: Int -> [Int] -> [Int]
sumOf' sum [] = []
sumOf' sum (x:xs) = (sum + x) : sumOf' (sum + x) xs

sumsOf :: [Int] -> [Int]
sumsOf xs = sumOf' 0 xs
