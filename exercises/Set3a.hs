...
-- better: mapMaybe2 _ _ _ = Nothing
...
-- capitalize str = unwords $ map (\(s:xs) -> toUpper s:xs) $ words str
capitalize = unwords . map (\(s:xs) -> toUpper s:xs) . words
-- advantage of . over $: no need to use mention argument
------------------------------------------------------------------------------
...
-- idea: generate list of powers until new value > max
-- use iterate!!
...
{-
-- fail due to too much memory
generator 1 = [1]
generator n = generator (n - 1) ++ [n]
isPower k 1 = True
isPower k x = mod x k == 0 && isPower k ( div x k)
powers :: Int -> Int -> [Int]
powers k max = filter (isPower k) $ generator max
-}
...
-- take 1 xs better than template matching
(+|+) :: [a] -> [a] -> [a]
(x:xs) +|+ (y:ys) = x:[y]
(x:xs) +|+ [] = [x]
[] +|+ (x:xs) = [x]
...
--     Right t -> t + sumRights xs
--     Left t -> sumRights xs

sumRights = 
    let to_value (Right t) = t
        to_value (Left _) = 0
    in
        sum . map to_value
...
multiCompose [] x = x
multiCompose (fu:fs) x = fu.multiCompose fs $ x
...
interpreter' [] results x y = results
interpreter' (command:commands) results x y =         
    case command of 
        "printX" -> interpreter' commands (results ++ [show x]) x y
        "printY" -> interpreter' commands (results ++ [show y]) x y
        "up" -> interpreter' commands results x (y + 1)
        "down" -> interpreter' commands results x (y - 1)
        "left" -> interpreter' commands results (x - 1) y
        "right" -> interpreter' commands results (x + 1) y
