...
updateAt :: Int -> a -> [a] -> [a]
updateAt i x xs = let (ys, zs) = splitAt i xs in ys ++ x : tail zs
...
substring i j s = let (ys, _) = splitAt j s 
                  in let (_, zs) = splitAt i ys 
                  in zs
-- substring i j s = drop i (take j s)
-- I used splitAt too much! when we doesn't care about both end of split result, take/drop is better! 
------------------------------------------------------------------------------
...
palindromify s = if reverse s == s then s 
...
safeDiv x y 
    ...
    | otherwise = Nothing
{-
when encounter fixed value, template matching is better option
safeDiv x 0 = Nothing
