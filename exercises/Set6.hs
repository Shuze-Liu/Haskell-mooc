...
instance Eq Name where
  (==) (Name s1) (Name s2) = map Data.Char.toLower s1 == map Data.Char.toLower s2
...
instance Eq a => Eq (List a) where
  (==) (LNode x1 x1s) (LNode x2 x2s) = x1 == x2 && x1s == x2s
  (==) Empty Empty = True
  (==) _ _ = False
...
simplify (RationalNumber n1 d1) = 
  let val = gcd n1 d1 in
  RationalNumber (n1 `div` val) (d1 `div` val)
...
instance Num RationalNumber where
  ...
  abs (RationalNumber n1 d1) = RationalNumber (abs n1) d1
  signum (RationalNumber n1 d1) = RationalNumber (signum n1) (signum d1)
  fromInteger x = RationalNumber x 1
  negate (RationalNumber n1 d1) = RationalNumber (negate n1) d1
